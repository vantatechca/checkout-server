...
class Settings(BaseSettings):
    ...
    # Used by the hourly Celery beat task `expire_old_orders`. Keep >= your
    # longest payment window (Interac = 2880 min = 48h) so the scheduler does
    # not expire orders customers are still legitimately paying.
    ORDER_EXPIRY_HOURS: int = 48
    ...
    def DATABASE_URL(self) -> str:
        # If a full connection string is provided (e.g. Render's DB_URL),
        # normalize it to the async psycopg/asyncpg driver and use it as-is.
    ...
    def DATABASE_URL_SYNC(self) -> str:
        # Sync driver (used by Alembic / any blocking tooling).
        url = os.getenv("DB_URL", "").strip()
        if url:
            if url.startswith("postgres://"):
                url = url.replace("postgres://", "postgresql://", 1)
